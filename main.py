...
class GameDatabase:
    ...
    # Include all existing game database methods...
...
/tonmonitor stop - Stop monitoring
    ...
    async def spaminfo_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            await update.message.reply_text("❌ Usage: /spaminfo @username or user_id")
            return
        
        target_user = context.args[0]
        try:
            if target_user.startswith('@'):
                # Handle username (would need additional logic to resolve to user_id)
                await update.message.reply_text("❌ Please use user_id instead of username.")
                return
            else:
...
What do you want to know today?
    ...
    # Include all other existing methods (game_command, mystats_command, etc.)
...
💪 **Ready to help the community and host game tournaments!**
    ...
    # Include all other existing game methods...
    # [All previous game methods remain exactly the same]
